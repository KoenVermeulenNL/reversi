    ...
    for (int i = 0; i < amountOfCells; i++) {
        for (int j = 0; j< amountOfCells;j++) {
            ...
            if (i%2 == 0) {
                ...
                else ImageBoxDrawer.FillRectangle(Brushes.Black, X, Y, (int)cellWidth, (int)cellWidth);
            } else {
                if (j%2 == 0) ImageBoxDrawer.FillRectangle(Brushes.Black, X, Y, (int)cellWidth, (int)cellWidth);
...
void UpdateBoard() {
    ...
    for (int row = 0; row < squares.GetLength(0); row++) {
        for (int col = 0; col < squares.GetLength(1); col++) {
            ...
            Debug.Write($"{col}: {squares[row, col]}, ");
        }
        Debug.WriteLine("");
    }
